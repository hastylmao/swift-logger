...
function CustomSplitModal({ onClose, onSave }: CustomSplitModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
      <div className="bg-slate-900 w-full max-w-lg rounded-2xl max-h-[85vh] overflow-hidden flex flex-col">
        <div className="p-4 border-b border-slate-800 flex items-center justify-between shrink-0">
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900 shrink-0">
          <button
            onClick={handleSave}
            disabled={!name.trim()}
            className="w-full py-4 rounded-xl bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold text-lg shadow-lg shadow-green-500/25 disabled:opacity-50 disabled:shadow-none"
          >
            💾 SAVE CUSTOM SPLIT
          </button>
          {!name.trim() && (
            <p className="text-center text-yellow-500 text-sm mt-2">Enter a name for your split above</p>
          )}
        </div>
      </div>
    </div>
